...
// struct iter_entry {
//     size_t index;       // which rect are we at
//     struct rect rect;   // node rect
//     struct node *node;  // node
// };

// struct rtree_iter {
//     const struct rtree *tr;    // rtree reference, ALWAYS first field
//     struct rect rect;          // target search rect
//     size_t cap;                // same as rtree height
//     size_t len;                // which stack node are we at
//     struct iter_entry stack[]; // stack of nodes
// };

// // size_t rtree_iter_memsize(const struct rtree *tr) {
// //     size_t memsize = sizeof(struct rtree_iter);
// //     memsize += tr->height * sizeof(struct iter_entry);
// //     return memsize;
// // }
...
// // {
// //     memcpy(&iter->rect.min[0], min, sizeof(NUMTYPE)*DIMS);
// //     memcpy(&iter->rect.max[0], max?max:min, sizeof(NUMTYPE)*DIMS);
// //     iter->cap = iter->tr->height;
// //     iter->len = 0;
// //     if (iter-> > 0) {
// //         iter->stack[0].index = 0;
// //         iter->stack[0].rect = iter->tr->rect;
// //         iter->stack[0].node = iter->tr->root;
// //     }
// // }
...
// // struct rtree_iter *rtree_iter_search(const struct rtree *tr, 
// //     const NUMTYPE min[], const NUMTYPE max[])
// // {
// //     if (!tr) return NULL;
// //     size_t memsize = sizeof(struct rtree_iter);
// //     memsize += tr->height * sizeof(struct iter_entry);
// //     struct rtree_iter *iter = tr->malloc(memsize);
// //     if (!iter) return NULL;
// //     iter->tr = tr;
// //     memcpy(&iter->rect.min[0], min, sizeof(NUMTYPE)*DIMS);
// //     memcpy(&iter->rect.max[0], max?max:min, sizeof(NUMTYPE)*DIMS);
...
// //     if (tr->height > 0) {
// //         iter->stack[0].index = 0;
// //         iter->stack[0].rect = tr->rect;
// //         iter->stack[0].node = tr->root;
// //     }
// //     printf("%zu\n", memsize);
// //     return iter;
// // }
